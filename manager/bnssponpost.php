...
if($_POST['lmKorwil'] !='') 
	$vCrit .= " and a.fidmember='{$_POST['lmKorwil']}'  ";

 
 $vsql="select distinct a.* from tb_komisi a left join m_anggota b on a.fidmember=b.frefer where a.fkind='SPON'  ";
...
; 
 $('#lmKorwil').select2();
        ...
        buttons: [{
             					extend: 'excelHtml5',
								filename: 'BonusSponsor<?="$vPeriodOri"?>',
								title: 'Bonus Sponsor Periode <?="$vPeriodOri"?>'
						}	, 
			 				{ 
								extend: 'print',
								title: 'Bonus Sponsor Periode <?="$vPeriodOri"?>',
        ...
        <div class="row"> 
           ...
           <b>  Periode </b>  </div>
              <div class="col-lg-4">
               ...
               <?
               		 $vSQL="select distinct concat(a.fdfrom,'_',a.fdto) as fperiod, fdfrom, fdto from tb_komisi a  where a.fkind='SPON'  ";
					$dbin->query($vSQL);
...
<div class="row"> 
           ...
           <b>   Pebisnis </b>  </div>
               <div class="col-lg-4">
                    ...
                    <?
                    	$vSQL = "select distinct frefer, fnamarefer from m_anggota where frefer <>'' and frefer <> '-' order by fnamarefer";
						$db->query($vSQL);
    ...
    <div class="table-responsive">
        <table width="100%" border="0" class="table table-striped table-bordered" id="tblMain">
          ...
          <tr style="font-weight:bold">
            ...
            <td class="" width="8%" style="height: 24px" align="center"><strong>ID Pebisnis</strong></td>
            ...
            <td width="15%" align="center" style="height: 24px; display:none" class=""><strong>Status Sync</strong></td>
            <td width="15%" align="center" style="height: 24px" class="hide">Status Bayar</td>
            <td width="15%" align="center" style="height: 24px" class="hide">&radic;</td>
            </tr>
            </thead>
            <tbody>
          <? 
          ...
          <tr id="tr<?=$vIdSys?>">
            ...
            <td class="" nowrap="nowrap"><?=$db->f('fidmember')?></td>
            <td class="hide"><?=$db->f('fidbisnis')?></td>
            <td style="width: 23%" ><?
			     $vSQL = "select kode,nama from tmmember where kode='{$db->f('fidmember')}'";
				$oDBAMHT->query($vSQL);
				$oDBAMHT->next_record();
				echo $vNama = $oDBAMHT->f('nama');
				$vRemark = "$vKet untuk {$db->f('fidmember')} / $vNama";
            ...
            <td id="tdstat<?=$vIdSys?>" class="hide"><?=$vSPayText?></td>
            <td id="tdstat<?=$vIdSys?>" class="hide"><input  id="btnPay<?=$vIdSys?>" name="btnPay<?=$vIdSys?>" type="button" class="btn btn-info btn-xs" value="Tandai Terbayar" onClick="setPay('<?=$vIdSys?>','<?=addslashes($vRemark)?>')" <? if ($vStatPay=='1') echo 'disabled'; ?>></td>
            </tr>
           <? } ?>
          <tr class="hide">
            <td style="width: 5%" >&nbsp;</td>
