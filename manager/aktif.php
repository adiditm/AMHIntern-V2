...
<select class="form-control" style="width:80px;display:inline-block;" onchange="if(this.value) window.location.href=this.value;">
   ...
   for ($i=0;$i<$vPageCount;$i++) {
	   $idisp=$i;
	 if ($vOP=="post") $idisp=0;
     $vPageUrl="aktif.php?lmAktif=".$vAktif."&lmMship=".$vPrem."&uPage=".$idisp."&lmSort=".$vSort."&current=".$_REQUEST['current']."&menu=".$_REQUEST['menu']."&tfNoHP=".$_REQUEST['tfNoHP']."&tfKota=".$_REQUEST['tfKota']."&tfDepart=".$_REQUEST['tfDepart']."&tfNama=".$_REQUEST['tfNama'];
?>
