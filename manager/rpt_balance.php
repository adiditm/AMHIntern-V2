...
//$vsql .= " union ";
//$vsql .="select fidkorwil as fidmember,a.fnama, b.fsaldovcr, a.falamat,a.fprop,a.fkota,a.faktif from m_korwil a left join m_pebisnis b on a.fidbisnis= b.fidmember  ";
$vsql = "select * from ($vsql) as gab where 1 ";
...
<div align="left" class="table-responsive col-lg-8" style="min-height:270px" > 
		  ...
		  <table border="0" cellpadding="4" cellspacing="0"  align="left" >
            ...
            <tr>
              <td nowrap height="25" style="width: 14%" ><div align="left">ID Korwil/Sub, Pebisnis</div></td>
...
<div class="row" style="text-align:left">
  <div class="col-lg-12">
   <? if ($vCurrent=='mdm_transaction') { ?>      
       <? if (true || $oSystem->checkPriv($vUser,"mdm_listprof_detail")) { ?> <input alt="Koreksi Saldo" name="btKorekSal" type="button" class="btn btn-success btn-sm" id="btKorekSal" onClick="document.location.href='../manager/koreksi.php?current=<?=$vCurrent?>&op=<?=$vSpy?>'+CryptoJS.MD5(getValue().trim())+'&uMemberId='+getValue()" value="Koreksi Saldo Pebisnis &raquo;" onMouseovers="showhint('Koreksi Saldo '+getValue(), this, event, '220px')" style="margin-top:5px" >  <? } ?> 
  ...
  <? if ($vCurrent=='mdm_master_data') { ?>      
          <? if (true || $oSystem->checkPriv($vUser,"mdm_listprof_genealogi")) { ?> <input alt="Detail / Verifikasi Data" name="btDetail" type="button" class="btn btn-success btn-sm" id="btDetail" onClick="return MM_goToURL('parent','../manager/regkorwil.php?current=<?=$vCurrent?>&op=<?=$vSpy?>'+CryptoJS.MD5(getValue().trim())+'&uMemberId='+getValue());return document.MM_returnValue" value="Detail / Edit Korwil &raquo;" onMouseovers="showhint('Lihat detail /  Edit Member '+getValue(), this, event, '220px')" style="margin-top:5px" >  <? }  ?>
		  
		  
        <? if (true || $oSystem->checkPriv($vUser,"mdm_listprof_genealogi")) { ?> 
...
<select class="form-control" style="width:80px;display:inline-block;" onchange="if(this.value) window.location.href=this.value;">
   ...
   for ($i=0;$i<$vPageCount;$i++) {
	   $idisp=$i;
     $vPageUrl="rpt_balance.php?lmAktif=".$vAktif."&lmMship=".$vPrem."&uPage=".$idisp."&lmSort=".$vSort."&current=".$_REQUEST['current']."&menu=".$_REQUEST['menu']."&tfID=".$vID."&tfNama=".$vNama;
?>
    ...
    <table width="100%" border="0" align="left" cellpadding="1" cellspacing="0" class="table" style="background-color:white" >
      ...
      <tr style="color:;font-weight:bold"> 
        ...
        <td   height="26" style="width: 30px"> <div align="center" >ID Pebisnis </div></td>
        ...
        <td widith="10%"><div align="center"  >Saldo 
        ...
        <td width="10%" style="display:none" ><div align="center" > Tanggal Daftar</div></td>
        ...
        <td width="43" style="display:"><div align="left" >&radic;</div></td>
 ...
 <? if($vAktifList=='1') { ?>	
           ...
           <!--<br><button onClick="doDel('<?=$db->f('fidkorwil')?>','<?=$vIDSys?>')" type="button" class="btn btn-xs btn-danger"><i class="fa fa-ban"></i> Delete</button>-->
        ...
        <td  style="height: 39px"> 
          ...
          <div align="right"><span >
            ...
            <?=number_format($db->f('fsaldovcr'),0,",",".");?>
        ...
        <td  style="height: 39px;display:none" > 
        ...
        <td class="hide"  style="height: 39px;<? if ($db->f('fpriv')=='1') echo 'background-color:#ccc';?>"><div align="right">
          ...
          <?=number_format($vSaldoAcc,0,",",".")?>
              ...
              <td  style="height: 39px;display:" <? if($vFall==$db->f('fidkorwil')) echo "bgcolor=#0f0";?> ><span >

                

                <input class="classRB" id="rbSelected<?=$db->f('fidkorwil')?>" style="width:20px;height:20px" name="rbSelected" type="radio" value="<?=$db->f('fidmember')?>" onClick="checkStatus('<?=$db->f('faktif')?>','<?=$db->f('fstockist')?>')" <? if ($vChoosed==$db->f('fidmember')) echo 'checked'; ?> >
      ...
      <tr  > 
        ...
        <td colspan="4" align="right" style="font-weight:bold">
          ...
          Total Active Pebisnis: <?=$vRecordCount?>, Total Saldo = <?=number_format($vSalAll,0,",",".");?></td>
...
<? if ($vCurrent=='mdm_master_data') { ?>      
 ...
 <? if ($vCurrent=='mdm_master_data') { ?>      
       <? if (true || $oSystem->checkPriv($vUser,"mdm_listprof_detail")) { ?> <input alt="Detail / Verifikasi Data" name="btDetail3" type="button" class="btn btn-success btn-sm" id="btDetail3" onClick="return MM_goToURL('parent','../manager/regkorwil.php?current=<?=$vCurrent?>&op=<?=$vSpy?>'+CryptoJS.MD5(getValue().trim())+'&uMemberId='+getValue());return document.MM_returnValue" value="Detail / Edit Korwil &raquo;" onMouseovers="showhint('Lihat detail /  Edit Member '+getValue(), this, event, '220px')" style="margin-top:5px" >  <? } ?>        
