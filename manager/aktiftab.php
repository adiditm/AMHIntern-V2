...
$vsql="select * from m_anggota_tab where 1 ";
...
function doUnblock(pParam,pIDTr) {
   ...
   if (confirm('Are you sure to activate this member ('+pParam+')')) {
	  ...
	  var vURL='../manager/processing_ajax.php?current=<?=$vCurrent?>&op=unblocktab&od='+pParam;
	  ...
	  $.get(vURL,function(data){
		  ...
		  if (data=='success') {
			...
			document.location.href='../manager/aktiftab.php?op=&current=mdm_admin&menu=mdm_admin_verify';
...
*/
	...
	<div class="right_col" role="main">
		...
		<div><label>
		<h3>Data Tabungan Calon  Jamaah</h3></label></div>
...
<tr >
            ...
            <tr>
              ...
              <td colspan="2"><div align="left"><br>
                ...
                <input name="Submit2" type="button" class="btn btn-default" value="Reset" onClick="document.location.href='../manager/aktiftab.php?op=&current=mdm_admin&menu=mdm_admin_verify'" />
...
<? 
  $vSeq = '1';
  include("../manager/btnmenutab.php");
...
<select class="form-control" style="width:80px;display:inline-block;" onchange="if(this.value) window.location.href=this.value;">
   ...
   for ($i=0;$i<$vPageCount;$i++) {
	   $idisp=$i;
	 if ($vOP=="post") $idisp=0;
     $vPageUrl="aktiftab.php?lmAktif=".$vAktif."&lmMship=".$vPrem."&uPage=".$idisp."&lmSort=".$vSort."&current=".$_REQUEST['current']."&menu=".$_REQUEST['menu']."&tfNoHP=".$_REQUEST['tfNoHP']."&tfKota=".$_REQUEST['tfKota']."&tfDepart=".$_REQUEST['tfDepart']."&tfNama=".$_REQUEST['tfNama'];
?>
   ...
   <? } ?>
        ...
        <td  style="height: 39px;<? if ($db->f('fpriv')=='1') echo 'background-color:#ccc';?>" align="center"><?=$oPhpdate->YMD2DMY($db->f('ftgldepart'))?></td>
        ...
        <td nowrap align="left" class="" style="height: 39px;<? if ($db->f('fpriv')=='1') echo 'background-color:#ccc';?>"><?=$db->f('fidregistrar')?></td>
        ...
        <td  style="height: 39px" align="center"><? if (trim($vCheckDoc)!='') {?><a  target="_blank" href="<?=$vTTD?>"> <li class="fa fa-picture-o" style="font-size:28px"></li></a><? } ?></td>
        <td  style="height: 39px" align="center"><? if (trim($vCheckKTP)!='') {?><a  target="_blank" href="<?=$vCheckKTP?>"> <li class="fa fa-picture-o" style="font-size:28px"></li></a><? } ?></td>
