...
$vsql="select distinct a.* from m_anggota a left join tb_payhist b on a.fidmember=b.fidmember where b.ftanggal is not null  and a.faktif='1' ";
...
*/
	...
	<div class="right_col" role="main">
		...
		<div><label>
		<h3>Sponsorship Jamaah  <?=$_GET['lmKorwil']?></h3>
		</label></div>
		  ...
		  <table border="0" cellpadding="4" cellspacing="0"  align="left" >
            ...
            <tr >
              ...
              <td height="25"><div align="left">
                  ...
                  <input name="tfDepart" type="text" class="form-control" id="tfDepart" value="<?=$_GET['tfDepart']?>" />
...
<tr >
              <td style="width: 14%; height: 25px;" nowrap><div align="left">Referensi Pendaftar</div></td>
              <td style="height: 25px"><div align="left">
                  <select name="lmKorwil" class="form-control" id="lmKorwil">
                    <option value="" selected="selected">--All--</option>
                    <?
                    	$vSQL = "select distinct frefer, fnamarefer from m_anggota where faktif='1' and frefer <>'' and frefer <> '-' order by fnamarefer";
						$db->query($vSQL);
						while($db->next_record()) {
							$vIDKor=$db->f('frefer');
							$vNamaKor=$db->f('fnamarefer');
            ...
            <tr>
              ...
              <td colspan="2"><div align="left"><br>
                ...
                <input name="Submit2" type="button" class="btn btn-default" value="Reset" onClick="document.location.href='../manager/rpt_regspon.php?op=&current=mdm_admin&menu=mdm_admin_lapspon'" />
...
<select class="form-control" style="width:80px;display:inline-block;" onchange="if(this.value) window.location.href=this.value;">
   ...
   for ($i=0;$i<$vPageCount;$i++) {
	   $idisp=$i;
	 if ($vOP=="post") $idisp=0;
     $vPageUrl="rpt_regspon.php?lmAktif=".$vAktif."&lmMship=".$vPrem."&uPage=".$idisp."&lmSort=".$vSort."&current=".$_REQUEST['current']."&menu=".$_REQUEST['menu'];
?>
    ...
    <table width="100%" border="0" align="left" cellpadding="1" cellspacing="0" class="table" style="background-color:white" >
      ...
      <tr style="color:;font-weight:bold"> 
        ...
        <td width="450" class="" style="width:2000px !important"><div align="center" >Alamat 
        ...
        <td width="268"><div align="center" > Tanggal Aktif</div></td>
        <td width="150" align="center">Tgl Lunas</td>
   ...
   <? } ?>
        ...
        <td width="450" style="height: 39px" class="" >          
        <div align="left"><span >
        ...
        <td  style="height: 39px;<? if ($db->f('fpriv')=='1') echo 'background-color:#ccc';?>" align="center" nowrap><?
         $vSQL = "select ftanggal as ftglprocessed from tb_payhist where fidmember='".$db->f('fidmember')."' and (fkind='lunas' or fkind='flunas')";
		 $dbin->query($vSQL);
		 $dbin->next_record();
		 echo $oPhpdate->YMD2DMY($dbin->f('ftglprocessed'));
		?></td>
        <td  style="height: 39px;<? if ($db->f('fpriv')=='1') echo 'background-color:#ccc';?>" align="center" nowrap><?=$oPhpdate->YMD2DMY($db->f('ftgldepart'))?></td>
