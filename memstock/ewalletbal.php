...
if ($vPriv=='sponsor')
     ...
     $vJenis = 'korwil';
			...
			if($vJenis=='korwil')	{
			     $vsql="select a.fidbisnis from m_korwil a left join m_pebisnis b on b.fidmember=a.fidbisnis  where a.fidkorwil='$vUserChoosed' ";	
			    $oDB->query($vsql);
				$oDB->next_record();
              ...
              <select name="lmKor" id="lmKor" class="form-control">
				...
				<?
				   ...
				   while ($db->next_record()) {
					   ...
					   $vNama = $db->f('fnama');
				?>	
                 <option <? if ($vID==$_POST['lmKor']) echo 'selected';?> value="<?=$vID?>"><?=$vID?> - <?=$vNama?></option>
...
//				if ($vMarkDev !='') echo "OPBal:::".$vOpBal;
				...
				while ($db->next_record()) {
				   ...
				   $vKind=$db->f('fkind');
				   $vDeb=$db->f("fdebit");
				   $vBalOri=$db->f("fbalance");
				   $vBal=$vCred-$vDeb;
				   $vArrBal[$i]=$vOpBal+$vBal;
				   if ($i>0) {
					  $vArrBal[$i]=$vArrBal[$i-1] + $vBal; 
				   }
        ...
        <tr class="tbrekapfont">
          ...
          <td  align="center" nowrap="nowrap"><div align="right"><?=$vNo+$vOffset?><? //if($vMarkDev!='') echo "::$vIdSys"; ?></div></td>
          ...
          <td  align="center"><div align="left"><a <? if ($vCred >0) echo 'style="cursor:pointer"';?> onClick="<? if ($vCred >0) { ?>alert('PPh21: <?=number_format($vTax21,0,",",".")?>'); <? } ?> //showDet('<?=$oPhpdate->YMD2DMY($vDateList,"-")?>', '<?=$vDesc?>','<?=number_format($vCred,0,",",".");?>','<?=number_format($vDeb,0,",",".");?>','<?=number_format($vFullNex,0,",",".");?>','<?=number_format($vFullRO,0,",",".");?>','<?=number_format($vFullAll,0,",",".");?>','<?=$vTax?>')"  data-toggle="modals" data-target="#detailModals" >
                ...
                <?
                   ...
                   if (!in_array($vKind,array('koreksi')))
                    ...
                    $vDesc = str_replace("nett","",$vDesc);
					$vDesc = str_replace("nett  nett  nett  nett","nett",$vDesc);
					 $vDesc = str_replace("nett  nett  nett","nett",$vDesc);
					  $vDesc = str_replace("nett  nett","nett",$vDesc);
					echo $vDesc;  
          ...
          <td align="center"><div align="right">
            ...
            <?=number_format($vCred,0,",",".");?><? //if($vMarkDev !='') echo "<br>Prev:".$vArrBal[$i-1];?>
...
<?=number_format($vDeb,0,",",".");?>
          ...
          <td align="center"><div align="right">
<?// ="{$vArrBal[$i-1]} + $vBal $i"?>&nbsp;&nbsp;
<?=number_format($vBalOri,0,",","."); //number_format($vArrBal[$i],0,",","."); $i++;?>
